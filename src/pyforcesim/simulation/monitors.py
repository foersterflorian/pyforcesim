...
from pyforcesim.types import PlotlyFigure, INF
...
class Monitor:
    ...
    def set_state(
        ...
        if self.state_status[target_state] and target_state != 'TEMP':
            loggers.monitors.info(f"Tried to set state of {self._target_object} to >>{target_state}<<, but this state was already set.\
        ...
        loggers.monitors.debug(f"Duration for state {current_state} on {self._target_object} was {current_state_duration}")
    ...
    def draw_state_bar_chart(        
        ...
        calc_td = _dt_mgr.timedelta_from_val(val=1., time_unit=time_unit)
...
class StorageMonitor(Monitor):
    ...
    def _track_fill_level(
        ...
        loggers.buffers.debug(f"[BUFFER: {self._target_object}] Current time is {current_time} with level {len(self._target_object)} and old level {self._current_fill_level}")
        #if ((self._current_fill_level != len(self)) and (duration > 0.0)) or is_finalise:
        if (self._current_fill_level != self._target_object.fill_level
            or is_finalise):
...
class InfStructMonitor(Monitor):
    ...
    def draw_WIP_level(
        ...
        if use_num_jobs_metric:
            ...
            # Plotly can not handle Timedelta objects properly, only Datetimes
            calc_td = _dt_mgr.timedelta_from_val(val=1., time_unit=time_unit_load_time)
